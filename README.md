...
- **Sección expandida, no fragmento**: cuando el pin de entidades trae una pieza del chunker, el retrieval la cambia por su **sección completa** (el padre por `metadata.chunk_id`, 9.378 chars en el caso real §4.6.2) y el filtro de recorte toma encabezado + ventanas alrededor de lo que la pregunta nombra, no `texto[:N]`. Si la pregunta nombra una categoría, los chunks de fuentes de **otra** categoría (`Rec2_Cat10`) no ocupan el top. El chunker además fusiona piezas diminutas de tabla y no arranca el overlap a mitad de palabra — [docs/architecture/evidence-first-gate.md §12](docs/architecture/evidence-first-gate.md)
- **Turn intent (capa conversacional)**: un saludo, agradecimiento, despedida, queja, comentario social o pregunta de capacidad **no necesita evidencia documental**. Reglas para lo obvio y JEV `PRE_RETRIEVAL` para la ambigüedad (`conversation_intent` + `needs_external_evidence`), con la **distribución completa** de JEV y una política que conserva la intención material secundaria (`«hola, explícame el byte 105»` sigue buscando fuentes). En un turno directo no hay retrieval, el `evidence_sufficiency` no aplica (no falla) y el gate documental se registra `not_applicable`: nunca más `INSUFFICIENT_ANSWER` por decir «hola». Pregunta de capacidad se responde con la configuración real del agente; `RAG_RUNTIME_TURN_INTENT=off` vuelve al comportamiento previo — [docs/architecture/turn-intent.md](docs/architecture/turn-intent.md)
