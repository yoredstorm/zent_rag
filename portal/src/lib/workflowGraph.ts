...
export const PURPOSE_GROUPS: PurposeGroup[] = [
  ...
  { id: "decide", label: "Decidir", types: ["ai_decision", "condition", "filter", "for_each"] },
...
export const NODE_LIBRARY: Record<string, NodeMeta> = {
  ...
  ai_decision: {
    ...
    ports: { input: P_IN, output: P_COND },
    fields: [
      { key: "decision_kind", label: "Tipo", type: "select", options: [
        { value: "route", label: "Elegir ruta" },
        { value: "yes_no", label: "Sí / No" },
        { value: "score", label: "Evaluar puntuación" },
      ], default: "route" },
      { key: "question", label: "Pregunta", type: "textarea", placeholder: "Determinar si el cliente requiere revisión manual" },
      { key: "confidence_min", label: "Confianza mínima", type: "number", default: 0.65, adv: true },
      { key: "on_low_confidence", label: "Si confianza baja", type: "select", options: [
        { value: "fallback", label: "Respaldo" },
        { value: "human_review", label: "Revisión humana" },
        { value: "stop", label: "Detener" },
        { value: "llm", label: "Agente" },
      ], default: "fallback", adv: true },
    ],
    defaults: { decision_kind: "route", confidence_min: 0.65, on_low_confidence: "fallback" },
    summary: (c) => String(c.question || "pregunta de negocio").slice(0, 48),
  },
  for_each: {
    type: "for_each",
    category: "logic",
...
const OUTPUT_FIELDS: Record<string, string[]> = {
  ...
  for_each: ["items_processed", "results", "errors", "duration_ms", "warnings"],
  join: ["merged", "branches", "values"],
  merge: ["first", "values", "strategy", "selected_from"],
  filter: ["filtered", "count", "total"],
  set_variable: ["variable", "value"],
  condition: ["result", "condition"],
  ai_decision: ["result", "choice", "route", "confidence", "low_confidence"],
  human_approval: ["approval_id", "status", "action", "summary"],
  marketplace_action: ["evidence_id", "cached", "cost", "latency_ms", "renderer"],
  business_node: ["result_id", "evidence_ids", "total_cost", "title", "action_0"],
  business_result: ["result_id", "importance"],
  trigger_schedule: [],
  trigger_webhook: [],
  trigger_event: [],
};
