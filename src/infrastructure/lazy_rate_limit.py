...
# =============================================================================
# Limita la frecuencia de triggers de lazy ingestion por tenant para evitar
...
def _rate_key(tenant_id: UUID) -> str:
    return f"rag:lazy_rl:{tenant_id.hex}"
...
async def lazy_trigger_allowed(tenant_id: UUID) -> bool:
    """True si el tenant aún puede disparar lazy ingestion esta hora."""
    settings = get_settings()
    max_triggers = settings.RAG_LAZY_INGEST_MAX_TRIGGERS_PER_HOUR
    try:
        client = await _get_redis()
        raw = await client.get(_rate_key(tenant_id))
        ...
        return _mem_get(_rate_key(tenant_id)) < max_triggers


async def record_lazy_trigger(tenant_id: UUID) -> int:
    ...
    try:
        ...
        return _mem_incr(_rate_key(tenant_id), window)


async def lazy_trigger_limited(tenant_id: UUID) -> bool:
    """True si el tenant está actualmente sobre el límite (para UI/API)."""
    settings = get_settings()
    max_triggers = settings.RAG_LAZY_INGEST_MAX_TRIGGERS_PER_HOUR
    try:
        client = await _get_redis()
