...
from src.api.metrics import rag_rerank_latency, rag_rerank_top_score
from src.config import get_settings
from src.domain.entities import RetrievalChunk
...
class LLMReranker:
    ...
    async def rerank(
        ...
        if tenant_id:
            rag_rerank_latency.labels(tenant_id=tenant_id).observe(elapsed)
            if result:
                rag_rerank_top_score.labels(tenant_id=tenant_id).set(result[0].score)
