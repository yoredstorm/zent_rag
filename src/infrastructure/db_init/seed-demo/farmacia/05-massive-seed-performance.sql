...
-- =============================================================================
INSERT INTO farmacia.categories (organization_id, name, slug, description, parent_id, display_order)
SELECT
...
-- =============================================================================
INSERT INTO farmacia.suppliers (organization_id, name, rut, contact_name, contact_phone, contact_email, website)
SELECT
    '00000000-0000-0000-0000-000000000001',
    lab.name,
...
INSERT INTO farmacia.products (id, organization_id, category_id, supplier_id, name, slug, description, sku, active_ingredient, concentration, presentation_unit, requires_prescription, price, cost, tags)
SELECT
    ...
    END,
    (SELECT id FROM farmacia.suppliers WHERE organization_id = '00000000-0000-0000-0000-000000000001' OFFSET (g % 30) LIMIT 1),
...
-- =============================================================================
INSERT INTO farmacia.product_images (organization_id, product_id, base64_data, mime_type, is_primary, sort_order)
SELECT
    '00000000-0000-0000-0000-000000000001',
    p.id,
    img.data,
...
-- =============================================================================
INSERT INTO farmacia.inventory (organization_id, product_id, warehouse_location, batch_number, expiration_date, quantity_available, quantity_reserved, quantity_minimum, last_restock_date, next_restock_eta_days)
SELECT
    '00000000-0000-0000-0000-000000000001',
    p.id,
    wh.name,
...
-- 5. CLIENTES (25,000)
-- =============================================================================
INSERT INTO farmacia.customers (organization_id, external_id, rut, name, email, phone, city, region, birth_date, health_insurance_id, loyalty_tier, created_at)
SELECT
    '00000000-0000-0000-0000-000000000001',
...
-- =============================================================================
INSERT INTO farmacia.prescriptions (organization_id, prescription_code, patient_rut, patient_name, doctor_name, doctor_rut, diagnosis, observations, issue_date, expiration_date)
SELECT
    '00000000-0000-0000-0000-000000000001',
...
INSERT INTO farmacia.sales (organization_id, product_id, customer_id, quantity, unit_price, total_amount, payment_method, payment_amount, health_insurance_id, insurance_covered_amount, prescription_id, order_status, sale_date, channel)
SELECT
...
CROSS JOIN LATERAL (
    SELECT ARRAY(SELECT id FROM farmacia.health_insurance WHERE organization_id = '00000000-0000-0000-0000-000000000001' ORDER BY id) AS hi_ids
) AS hi
CROSS JOIN LATERAL (
    SELECT ARRAY(SELECT id FROM farmacia.prescriptions WHERE organization_id = '00000000-0000-0000-0000-000000000001' ORDER BY id) AS rx_ids
) AS rx;
...
INSERT INTO farmacia.product_reviews (organization_id, product_id, customer_id, rating, title, comment, is_verified_purchase, helpful_count, created_at)
SELECT
    '00000000-0000-0000-0000-000000000001'::uuid,
    prod_ids[1 + (g % 50000)],
...
-- =============================================================================
-- Orden: primero la materializada; DROP VIEW sobre una materialized view
-- falla con ON_ERROR_STOP=1 en re-seeds del mismo volumen.
