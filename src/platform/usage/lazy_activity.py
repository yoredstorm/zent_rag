...
def lazy_log_cache_key(organization_id: UUID) -> str:
    return f"rag:lazy:log:{organization_id.hex}"


def lazy_rows_cache_key(organization_id: UUID, schema: str, table: str) -> str:
    return f"rag:lazy_rows:{organization_id.hex}:{schema}.{table}"


def preferred_organization_id(auth_organization_id: str | None, header_organization_id: str | None) -> str:
    """El organization del Bearer gana; el header solo se usa si no hay auth."""
    return (auth_organization_id or "").strip() or (header_organization_id or "").strip()
