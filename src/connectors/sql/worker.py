...
from src.connectors.sql.queue import QUEUE_KEY, update_job_status
...
async def process_trigram_index_job(job_id: str, job_data: dict) -> None:
    ...
    from src.infrastructure.postgres.relational_db import get_engine
...
async def process_job(job_data: dict) -> None:
    ...
    organization_id = UUID(job_data["organization_id"])

    # Restaurar el TenantContext del job en el ContextVar: SOLO identifica la
    # organización (sin roles/permisos elevados). El organization_id del job
    # lo escribe el API desde la identidad autenticada; el worker nunca lo
    # acepta de fuentes externas al sistema.
    ...
    try:
        await _process_job_inner(job_id, organization_id, job_data)
    finally:
        clear_tenant_context()


async def _process_job_inner(
    job_id: str, organization_id: UUID, job_data: dict
) -> None:
