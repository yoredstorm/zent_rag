...
from src.platform.usage.lazy_activity import lazy_rows_cache_key
...
class PostgresIngestionService(IngestionService):
    ...
    def _sync_key(self, organization_id: UUID, schema: str, table: str) -> str:
        return f"rag:synced:{organization_id.hex}:{schema}.{table}"

    async def is_synced(self, organization_id: UUID, schema: str, table: str) -> bool:
        if not self._cache:
            return False
        return await self._cache.exists(self._sync_key(organization_id, schema, table))

    async def get_lazy_rows_indexed(self, organization_id: UUID, schema: str, table: str) -> int:
        if not self._cache:
            return 0
        raw = await self._cache.get(lazy_rows_cache_key(organization_id, schema, table))
        try:
            return int(raw) if raw else 0
        except (TypeError, ValueError):
            return 0

    async def get_sync_statuses(self, organization_id: UUID, sources: list[DataSource]) -> list[dict]:
        results = []
        for s in sources:
            synced = await self.is_synced(organization_id, s.schema_name, s.table_name)
            progress = await self.get_table_progress(organization_id, s.schema_name, s.table_name)
    ...
    async def get_table_progress(self, organization_id: UUID, schema: str, table: str) -> dict | None:
        if not self._cache:
            return None
        import json
        raw = await self._cache.get(f"rag:progress:{organization_id.hex}:{schema}.{table}")
        if raw:
            try:
    ...
    async def discover_sources(self, organization_id: UUID) -> list[DataSource]:
        """Descubre todas las tablas indexables para un organization."""
    ...
    async def sync_all(
        self, organization_id: UUID, full_refresh: bool = False, job_id: str | None = None
    ) -> IngestionResult:
        """Sincroniza todas las tablas automáticamente (con concurrencia configurable)."""
        start = time.perf_counter()
        sources = await self.discover_sources(organization_id)
        active_sources = [
            s for s in sources
        ...
        if job_id:
            from src.connectors.sql.queue import update_job_status
        ...
        async def _run_one(source: DataSource) -> IngestionResult:
            ...
            if has_updated_at and self._cache and not full_refresh:
                since_ts = await self._cache.get(
                    self._sync_ts_key(organization_id, source.schema_name, source.table_name)
                )
                if since_ts:
                    logger.info("Incremental sync", table=table_label, since=since_ts)

            table_result = await self._ingest_table(
                organization_id, source, job_id=job_id, since_timestamp=since_ts
            )
    ...
    def _sync_ts_key(self, organization_id: UUID, schema_name: str, table_name: str) -> str:
    ...
    async def sync_table(
        ...
        sync_ts_key = self._sync_ts_key(organization_id, schema_name, table_name)
    ...
    async def _ingest_table(
        ...
        try:
            ...
            has_organization_col = any(c.name == "organization_id" for c in source.columns)
            ...
            while True:
                ...
                if self._cache and source.row_count and source.row_count > 0:
                    ...
                    await self._cache.set(
                        f"rag:progress:{organization_id.hex}:{schema}.{table}",
                        json.dumps({
            ...
            if self._cache and result.rows_indexed > 0:
                await self._cache.set(
                    self._sync_key(organization_id, schema, table), "1", ttl_seconds=86400
                )
                import json
                await self._cache.set(
                    f"rag:progress:{organization_id.hex}:{schema}.{table}",
    ...
    async def _discover_image_table(
        self, session: AsyncSession, schema: str, table: str
    ) -> tuple[str, str, str] | None:
        """Descubre (genéricamente) una tabla de imágenes referenciada por FK.

        Busca tablas con columnas típicas de imagen (base64_data + is_primary)
        que tengan FK hacia la tabla fuente. Retorna (schema, tabla, columna FK)
        o None. Sin nombres hardcodeados de negocio.
        """
        try:
            rows = await session.execute(
                text(
                    "SELECT DISTINCT fk.table_schema, fk.table_name, fk.column_name "
                    ...
                    " AND tc.table_schema = ref.table_schema "
                    "WHERE tc.constraint_type = 'FOREIGN KEY' "
                    "  AND ref.table_schema = :schema "
                    "  AND ref.table_name = :table "
                    "  AND EXISTS ("
                    "    SELECT 1 FROM information_schema.columns c "
                    "    WHERE c.table_schema = fk.table_schema "
                    "      AND c.table_name = fk.table_name "
                    "      AND c.column_name = 'base64_data'"
                    "  ) "
                    "LIMIT 1"
                ),
    ...
    async def _load_product_images(
        ...
        image_ref = await self._discover_image_table(session, schema, table)
        if image_ref is None:
            return product_images
        img_schema, img_table, fk_column = image_ref
        img_schema_q = quote_ident(img_schema)
        img_table_q = quote_ident(img_table)
        fk_q = quote_ident(fk_column)
        try:
            img_rows = await session.execute(
                text(
                    f"SELECT {fk_q}::text, base64_data "
                    f"FROM {img_schema_q}.{img_table_q} "
        ...
        *,
        ...
        for idx, (doc_id, content_text, meta) in enumerate(
            ...
            if len(batch_points) >= self._upsert_batch_size:
                try:
                    await self._vector_store.upsert_batch(organization_id, batch_points)
    ...
    async def ingest_candidates(
        ...
        start = time.perf_counter()
        result = IngestionResult(organization_id=organization_id, tables_processed=0)
        try:
    ...
    async def _ingest_candidates_impl(
        ...
        if not keywords:
            logger.info("Lazy ingestion skipped: no keywords", organization_id=str(organization_id))
            return result
        ...
        for source in sources:
            ...
            if role == "customer" and source.is_view:
                continue
            if await self.is_synced(organization_id, source.schema_name, source.table_name):
                continue
        ...
        try:
            ...
            for source in candidates:
                try:
                    ...
                    cooldown_key = (
                        f"rag:lazy_cooldown:{organization_id.hex}:"
                    ...
                    if not rows:
                        continue
                    fk_resolutions = await self._build_fk_resolutions(session, source)
                    product_images = await self._load_product_images(
                        session, source.schema_name, source.table_name
                    )
    ...
    async def _ensure_trigram_index_background(
        self, organization_id: UUID, schema: str, table: str, columns: list[str]
    ) -> None:
        ...
        if not columns:
            return
        try:
            from src.connectors.sql.queue import enqueue_trigram_index
